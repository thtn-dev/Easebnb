...
public sealed class ChangePictureProfileEndpoint : IEndpointGroup
{
        ...
        [FromServices] IAccountService accountService, CancellationToken ct)
    {
        ...
        var validation = await file.ValidateAsync(avatarUploadSettings.Value.MaxFileSizeBytes, FileSignatures.ImageExtensions, ct);
        if (!validation.IsValid)
            return Results.BadRequest(new { error = validation.Error });
        
        await using var safeImage = await file.ToSafeJpegAsync(maxDimension: avatarUploadSettings.Value.MaxDimension, ct: ct);
